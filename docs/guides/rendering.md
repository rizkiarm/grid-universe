...
- Selection rules:
    ...
    - Background: from all background items, pick the one with the lowest priority after sorting descending (i.e., visually most “behind”; your code sets floor priority=10 and wall priority=9 by default).
...
- Object properties influence asset lookup:
    ...
    - Every entity’s “appearance name” (AppearanceName) and the set of component “properties” (e.g., "pushable", "pathfinding", "dead", "locked", "required", etc.) are used to find the best-matching texture from a texture map (see next section).
...
- Best-match selection:
    ...
    - This allows falling back to a generic “BOX” when (“pushable”,) texture is not provided, and specializing when available.
...
- Coloring:
    ...
    - apply_recolor_if_group(image, group) recolors the texture to the group’s hue while preserving the per-pixel value (brightness/tone) and by default preserving saturation (configurable).
...
```python
from grid_universe.renderer.texture import render, TextureRenderer, DEFAULT_TEXTURE_MAP, DEFAULT_GROUP_RULES, GroupRule
...
# You can copy render() and inject a custom rules list into derive_groups(...) call,
# or vend your own helper that wraps TextureRenderer with a patched render.
# For compactness, we’ll override TextureRenderer.render via composition:

class BoxGroupingRenderer(TextureRenderer):
    def render(self, state: State):
        from grid_universe.renderer.texture import derive_groups
        ...
        # Use the same underlying render pipeline but pre-inject groups via a custom tex_lookup
        # (short approach: monkey-patch derive_groups in a local copy; long approach shown here is conceptual)
        return super().render(state)

# For a full override, replicate renderer.render() and substitute groups=derive_groups(...)
# with your custom rules. This snippet shows the idea.
```
...
- “Group coloring looks odd or too saturated.”
    ...
    - apply_recolor_if_group preserves tone (value), but you can adjust saturation behavior by modifying recolor_image_keep_tone parameters (keep_saturation, saturation_mix, min_saturation) inside renderer.utils.image if you expose or wrap them.
...
- “Performance is slow when rendering many frames.”
    ...
    - Reduce the number of moving overlays (move_speed triangles) if they change frequently; overlays are part of the cache key.
