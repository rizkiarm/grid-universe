...
- State is frozen (dataclasses.dataclass(frozen=True)); systems produce a brand-new State using dataclasses.replace.
...
- pathfinding_system(state) -> state
    ...
    - Rules:
        ...
        - If the pathfinding target has active Phasing, a usage tick can be consumed and pursuit may be skipped for this step (per code).
